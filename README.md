...
| **GHOST TRAIN** | Under-field subway behind the **turnstile spinner** — a right-flipper cross shot. The turnstile *gates* the dark ride: it starts closed each ball, and spinning it a couple of times opens it for one ride, which then closes it again (a green mouth lamp shows when it's open). A ride scores 2,500 pts + 500 bonus units and punches the pass. |
