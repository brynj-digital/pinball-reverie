...
Milestones 0–4 built: Vite + TypeScript + Planck.js; Canvas-2D renderer behind the `Renderer` interface; scrolling camera; full element set (flippers, bumpers, slings, drop-target bank, rollovers, orbit + spinner) scoring via the EventBus; and the M4 layer — game flow (attract → 3-ball play → game over), ball-saver, nudge/tilt, moon-lane bonus multipliers, localStorage high scores, and the DMD subsystem (`src/render/dmd/`: `DotMatrix` 128×32 amber surface + 5×7 bitmap font, `DmdScene`, `DmdQueue`; Game owns it, renderers only composite its canvas). The build plan at `docs/plans/pinball-build-plan.md` remains the source of truth — **read it before any implementation work**. Flipper feel is player-approved; physics tuning stays exposed in the debug panel.
