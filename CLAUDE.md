...
Milestones 0–5 built: Vite + TypeScript + Planck.js; Canvas-2D renderer behind the `Renderer` interface; scrolling camera; full element set (flippers, bumpers, slings, drop-target bank, rollovers, orbit + spinner) scoring via the EventBus; and the M4/M5 layer — game flow (attract → 3-ball play → game over), ball-saver, nudge/tilt, moon-lane bonus multipliers, localStorage high scores, the DMD subsystem (`src/render/dmd/`: `DotMatrix` 128×32 amber surface + 5×7 bitmap font, `DmdScene`, `DmdQueue`; Game owns it, renderers only composite its canvas), baked DMD event animations (SVG sprite-strip masters in `design/dmd-scenes/` quantized to dot frames at load by `dmd/bake.ts` — never live SVG on the grid), and in-world juice (camera shake, pooled impact rings, ball trail). The build plan at `docs/plans/pinball-build-plan.md` remains the source of truth — **read it before any implementation work**. Flipper feel is player-approved; physics tuning stays exposed in the debug panel.
